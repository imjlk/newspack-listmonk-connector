	...
	function newspack_listmonk_connector_campaign_analytics_build_response( array $payload ) {
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new WP_Error(
				...
				__( 'You are not allowed to read analytics for this newsletter.', 'wp-typia-newsletter-connector' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( ! $campaign_id ) {
			return new WP_Error(
				...
				__( 'This newsletter has not been synced to a Listmonk campaign yet.', 'wp-typia-newsletter-connector' ),
				array( 'status' => 409 )
			);
		}
