...
if ( ! function_exists( 'newspack_listmonk_connector_newsletter_sync_load_rest_resource_schema' ) ) {
	...
	function newspack_listmonk_connector_newsletter_sync_load_rest_resource_schema( $schema_name ) {
		...
		$schema_path  = $project_root . '/src/rest/newsletter-sync/api-schemas/' . $schema_name . '.schema.json';
	...
	function newspack_listmonk_connector_newsletter_sync_get_provider() {
		if ( ! class_exists( 'Newspack_Newsletters' ) ) {
			return new WP_Error(
				...
				__( 'Newspack Newsletters is not available.', 'newspack-listmonk-connector' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $provider instanceof Newspack_Listmonk_Connector_Provider ) {
			return new WP_Error(
				...
				__( 'The Listmonk provider is not available.', 'newspack-listmonk-connector' ),
				array( 'status' => 500 )
			);
		}
	...
	function newspack_listmonk_connector_newsletter_sync_build_response( array $payload ) {
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new WP_Error(
				...
				__( 'You are not allowed to sync this newsletter.', 'newspack-listmonk-connector' ),
				array( 'status' => 403 )
			);
		}
