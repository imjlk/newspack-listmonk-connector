...
final class Newspack_Listmonk_Connector_Provider extends Newspack_Newsletters_Service_Provider {
	...
	public static function get_labels( $context = '' ) {
		return array_merge(
			...
			array(
				'name'       => 'Listmonk',
				'list'       => __( 'list', 'wp-typia-newsletter-connector' ),
				'lists'      => __( 'lists', 'wp-typia-newsletter-connector' ),
				'List'       => __( 'List', 'wp-typia-newsletter-connector' ),
				'Lists'      => __( 'Lists', 'wp-typia-newsletter-connector' ),
				'tag_prefix' => 'Newspack: ',
			)
	...
	public function set_api_credentials( $credentials ) {
		...
		if ( empty( $settings['base_url'] ) || empty( $settings['api_user'] ) || empty( $settings['api_token'] ) ) {
			return new WP_Error(
				...
				__( 'Please enter the Listmonk API URL, user, and token.', 'wp-typia-newsletter-connector' )
			);
		}
	...
	public function test( $post_id, $emails ) {
		...
		if ( empty( $emails ) ) {
			return new WP_Error(
				...
				__( 'Please enter at least one valid test email address.', 'wp-typia-newsletter-connector' )
			);
		}
		...
		return array(
			'message' => sprintf(
				...
				__( 'Listmonk test message sent to %s.', 'wp-typia-newsletter-connector' ),
				implode( ', ', $emails )
			),
			'result'  => $result,
		);
	...
	public function sync( $post ) {
		if ( ! $this->has_api_credentials() ) {
			return new WP_Error(
				...
				__( 'Listmonk API credentials are missing.', 'wp-typia-newsletter-connector' )
			);
		}
		if ( empty( $post->post_title ) ) {
			return new WP_Error(
				'newspack_listmonk_connector_empty_subject',
				__( 'The newsletter subject cannot be empty.', 'wp-typia-newsletter-connector' )
			);
		}
		...
		if ( empty( $payload['lists'] ) ) {
			return new WP_Error(
				...
				__( 'Please select at least one Listmonk list.', 'wp-typia-newsletter-connector' )
			);
		}
		...
		if ( is_wp_error( $result ) ) {
			...
			set_transient(
				...
				__( 'Listmonk sync error: ', 'wp-typia-newsletter-connector' ) . $result->get_error_message(),
				45
			);
			return $result;
		}
	...
	public function send( $post ) {
		...
		if ( is_wp_error( $sync ) ) {
			...
			set_transient(
				$this->get_sync_error_transient_name( $post->ID ),
				__( 'Listmonk send error: ', 'wp-typia-newsletter-connector' ) . $sync->get_error_message(),
				45
			);
			return $sync;
		}
		...
		if ( is_wp_error( $result ) ) {
			...
			set_transient(
				$this->get_sync_error_transient_name( $post->ID ),
				__( 'Listmonk send error: ', 'wp-typia-newsletter-connector' ) . $result->get_error_message(),
				45
			);
			return $result;
		}
	...
	public function add_contact( $contact, $list_id = false ) {
		...
		if ( ! $subscriber_id ) {
			return new WP_Error(
				...
				__( 'Listmonk subscriber response did not include a valid ID.', 'wp-typia-newsletter-connector' )
			);
		}
	...
	public function get_usage_report( $post_id = 0, $from = '', $to = '' ) {
		...
		if ( ! $post_id ) {
			return new WP_Error(
				...
				__( 'A synced newsletter post ID is required to read Listmonk usage data.', 'wp-typia-newsletter-connector' ),
				array( 'status' => 409 )
			);
		}
	...
	private function subscriber_blocklisted_error( $email, array $subscriber ) {
		return new WP_Error(
			...
			__( 'The Listmonk subscriber is blocklisted and must be reviewed in Listmonk before it can be resubscribed.', 'wp-typia-newsletter-connector' ),
			array(
	...
	private function archive_post_campaign( $post_id, $event ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			set_transient(
				...
				__( 'Listmonk archive error: ', 'wp-typia-newsletter-connector' ) . $result->get_error_message(),
				45
			);
			return;
		}
	...
	private function not_implemented( $message = null ) {
		return new WP_Error(
			...
			$message ? $message : __( 'This Listmonk provider method is not implemented yet.', 'wp-typia-newsletter-connector' )
		);
	}
	...
	private function tags_not_supported_error() {
		return new WP_Error(
			...
			__( 'Listmonk subscriber tags are not mapped to Newspack local tags. Use Listmonk lists, subscriber attributes, or campaign template merge tags instead.', 'wp-typia-newsletter-connector' )
		);
	}
}
